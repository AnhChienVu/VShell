...
5. Link the package globally:
   ...
   1. Ensure Your Project is Set Up:
      Make sure you have your project files ready, including `server.js` and `.env`.
   2. Add a `bin` Field to `package.json`:
      Ensure your `package.jso`n has a `bin` field that points to your `server.js` file. Here’s an example:
...
6. To run VShell, use the following command:

   `vshell file_name(s) <arguements>`

7. Create a `.env` file to store your Groq API key and other necessary configuration values.
